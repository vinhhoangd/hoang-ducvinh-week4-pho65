...
| Yuhe |  | *Verifier:* independently re-run the known-answer sequence on your own machine and confirm 8 / $2.25 / $18.00 without copying Vinh's numbers |  |  |
| yuhe |  | *Verifier:* run the phone test (camera, saved image, manual) and record every row of the mobile test |  |  |
| Shiming |  | *Business reviewer:* answer the moving-average prompts in your own words and sign the continue/revise/stop decision |  |  |
| Shiming |  | *Business reviewer:* follow the intern handoff uncoached, record where you hesitated, then re-test after the revision |  |  |
| Shiming |  | *Ubuntu machine:* generate your own key, add the `.pub` to `authorized_keys`, and confirm `whoami` → `pho65user` there |  |  |
...
| `evidence/mobile-transfer-test.md` | Yuhe | in progress at Step 8 |
| `evidence/intern-workflow-handoff.md` | Shiming | awaiting the uncoached test |
| `evidence/decision-table.md` Part B | Shiming | awaiting each member's own answers |
| `README.md` | Vinh | drafted; business reviewer edits Section 9 |

## Next action

**Owner: Vinh** — hand off to Yuhe for the ngrok phone test after Yuhe's public key is installed and the app is confirmed healthy locally.
